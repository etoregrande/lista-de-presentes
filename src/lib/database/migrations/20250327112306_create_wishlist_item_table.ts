...
export async function up(db: Kysely<any>): Promise<void> {
  await db.schema
    ...
    .addColumn('name', 'varchar(255)', (col) => col.check(sql`length(name) >= 1`).notNull())
    .addColumn('price', 'integer', (col) => col)
    .addColumn('description', 'text', (col) => col)
    .addColumn('image', 'text', (col) => col)
    .addColumn('link', 'text', (col) => col)
    .addColumn('priority', 'varchar(10)', (col) => col.check(sql`priority IN ('alta', 'normal', 'baixa')`).defaultTo('normal'))
